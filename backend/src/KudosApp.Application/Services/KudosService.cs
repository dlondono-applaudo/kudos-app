...
public class KudosService : IKudosService
{
    ...
    public async Task<KudosResponse> CreateAsync(string senderId, CreateKudosRequest request)
    {
        ...
        var kudos = Kudos.Create(senderId, request.ReceiverId, request.CategoryId, request.Message, category.PointValue);
    ...
    public async Task<KudosFeedResponse> GetFeedAsync(int page, int pageSize)
    {
        ...
        var items = await _context.Kudos
            ...
            .Select(k => new KudosResponse(
                k.Id, k.SenderId, k.Sender.FullName, k.ReceiverId, k.Receiver.FullName,
    ...
    public async Task<KudosResponse?> GetByIdAsync(int id)
    {
        ...
        return kudos is null ? null : MapDirect(kudos);
    }
    ...
    {
        var totalPoints = await _context.Kudos.Where(k => k.ReceiverId == userId).SumAsync(k => k.Points);
        var existingBadgeIds = await _context.UserBadges.Where(ub => ub.UserId == userId).Select(ub => ub.BadgeId).ToListAsync();
        var newBadges = await _context.Badges.Where(b => b.RequiredPoints <= totalPoints && !existingBadgeIds.Contains(b.Id)).ToListAsync();
        ...
        if (newBadges.Count > 0) await _context.SaveChangesAsync();
    }
    ...
    private async Task<KudosResponse> MapToResponse(Kudos kudos)
    {
        ...
        return MapDirect(kudos);
    }

    private static KudosResponse MapDirect(Kudos kudos) =>
        new(kudos.Id, kudos.SenderId, kudos.Sender.FullName, kudos.ReceiverId, kudos.Receiver.FullName,
            kudos.Category.Name, kudos.Message, kudos.Points, kudos.SentimentEmoji, kudos.CreatedAt);
}
