...
public class OpenAiService : IAiSuggestionService, IContentModerationService
{
    ...
    public async Task<IReadOnlyList<string>> SuggestMessagesAsync(string categoryName, string intent)
    {
        ...
        try
        {
            var response = await _chatClient.CompleteChatAsync(
                new List<ChatMessage> { new SystemChatMessage(systemPrompt), new UserChatMessage($"Category: {categoryName}\nIntent: {intent}") },
    ...
    public async Task<ModerationResult> ValidateAndAnalyzeAsync(string message, string categoryName)
    {
        ...
        try
        {
            ...
            var content = response.Value.Content[0].Text.Trim();
            var result = JsonSerializer.Deserialize<ModerationJson>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });

            return result is null
                ? new ModerationResult(true, null, "🌟")
                : new ModerationResult(result.Approved, result.Reason, result.Emoji ?? "🌟");
        }
        catch (Exception ex)
        {
